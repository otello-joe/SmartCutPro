...
def get_video_full_info(filepath):
    try:
        ...
        w = int(data.get('width', 1920)); h = int(data.get('height', 1080))
    ...
    except: return 1920, 1080, 5000, 30.0, 0.0
...
def process_video_ffmpeg(in_video, out_video, start_t, end_t, wm_path, bgm_path, speed, vol, w, h, has_a, stop_event, pause_event, progress_cb, crop_filter=None, crf=23):
    ...
    def get_map(pad): return pad if pad.startswith("0:") else f"[{pad}]"
    ...
    if start_t is not None and end_t is not None: total_dur = (end_t - start_t) / speed
    else:
    ...
    error_log =[]
    try:
        kwargs = {'stdout': subprocess.DEVNULL, 'stderr': subprocess.PIPE, 'text': True, 'env': get_clean_env(), 'universal_newlines': True}
